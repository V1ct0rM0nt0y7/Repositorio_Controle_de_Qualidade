...
#### **Distribuição Estacionária da Cadeia**
...
+ $\phi (w,s) = c_{insp} + \varepsilon (w,s) + \theta (w,s) + \varphi (w,s)$
...
#### **Funções Utilizadas** {.tabset .tabset-pills}

##### **Funções Amostrais**

```{r funcoes1, echo = T, include = T, warning = F}
## Função 1: Função para calcular a média de um vetor numérico
...
## Função 2: Função para calcular a variância de um vetor numérico
...
## Função 3: Função para calcular a mediana de um vetor numérico
...
## Função 4: Função para gerar uma matriz identidade de tamanho n
gerar.matriz.identidade <- function(n) {
  identidade <- diag(1, n) ## Utiliza a função diag para criar uma matriz identidade de tamanho n
  return(identidade) ## Retorna a matriz identidade gerada
}

## Função 5: Função para transpor uma matriz
transpor.matriz <- function(matriz) {
  transposta <- t(matriz) ## Transpõe a matriz utilizando a função t
  return(transposta) ## Retorna a matriz transposta
}

## Função 6: Função para multiplicar duas matrizes
multiplicar.matriz <- function(matriz1, matriz2) {
  if (ncol(matriz1) != nrow(matriz2)) { ## Verifica se o número de colunas da primeira matriz é igual ao número de linhas da segunda
    stop("Dimensões incompatíveis para multiplicação.") ## Retorna um erro caso as dimensões sejam incompatíveis
  }
  produto <- matriz1 %*% matriz2 ## Realiza a multiplicação matricial utilizando o operador %*%
  return(produto) ## Retorna o resultado da multiplicação
}
```
...
## Função 7: Calcula a probabilidade de X = k para uma distribuição geométrica
...
## Função 8: Calcula a função de distribuição acumulada (CDF) para X <= k
...
## Função 9: Gera n valores aleatórios seguindo a distribuição geométrica
...
## Função 10: Calcula a média teórica da distribuição geométrica
...
## Função 11: Calcula a variância teórica da distribuição geométrica
...
## Função 12: Calcula p_(AS) - Probabilidade de Não Ajustar o processo Sob Controle
...
## Função 13: Calcula p_(AF) - Probabilidade de Não Ajustar o processo Fora de Controle
...
## Função 14: Calcula a probabilidade do processo continuar "Sob Controle" por L ciclos, dado que começa "Sob Controle"
...
## Função 15: Calcula a probabilidade do processo continuar "Sob Controle" por m ciclos, dado que começa "Sob Controle"
...
## Função 16: Calcula a probabilidade de o processo passar para "Fora de Controle" após L ciclos, dado que começa "Sob Controle"
...
## Função 17: Calcula a probabilidade de o processo passar para "Fora de Controle" após m ciclos, dado que começa "Sob Controle"
...
##### **Funções da Distribuição Estacionária**

```{r funcoes6, echo = T, include = T, warning = F}
calcular_pi <- function(P) {
  n <- nrow(P)  ## Número de estados
  I <- diag(n) ## Cria a matriz Identidade.
  
  P_minus_I <- P - I ## Substrae a matriz identidade de P
  
  # Adicionar a restrição de normalização (soma das probabilidades deve ser 1)
  # Para isso, adicionamos uma linha de 1's e uma coluna de 1's no sistema
  P_augmented <- rbind(P_minus_I, rep(1, n))
  
  # Agora, adicionamos a restrição de normalização, que é uma linha de 1's
  P_augmented <- cbind(P_augmented, c(rep(0, n - 1), 1))
  
  # Vetor de constantes (todas as constantes são 0, exceto a última que é 1)
  b <- c(rep(0, n), 1)
  
  # Resolver o sistema linear (P_augmented) %*% pi = b
  pi <- solve(P_augmented, b)
  
  return(pi)
}

calcular_p_L <- function(P, pi) {
  n <- nrow(P)
  p_L <- matrix(0, nrow = n, ncol = 1)
  
  for (i in 1:n) {
    if (i <= 3) {
      soma_pi <- sum(pi[1:3])
      p_L[i] <- sum(P[i, 1:3] * pi[1:3]) / pi[i]
    } else {
      p_L[i] <- 0
    }
  }
...
calcular.custos <- function(p_mais, p_menos, alfa, beta, c_inspecao, c_ajuste, 
                             c_desc_c, c_desc_nc, c_nc, p_AS, p_AF, p_L, pi, L, m) {
  ...
  # Cálculo para o estado (0,0)
  xi_00 <- c_nc * ((p_AF[1] * (m - 1) * (1 - p_mais)) + (p_L[1] * (L - 1) * (1 - p_mais)))
  theta_00 <- (c_desc_c * (p_mais * alfa) / (1 - p_AS)) + 
              (c_desc_nc * ((1 - p_mais) * (1 - beta)) / (1 - p_AS))
  phi_00 <- c_inspecao + xi_00 + theta_00 + c_ajuste
  
  # Cálculo para o estado (0,1)
  xi_01 <- c_nc * ((p_AF[2] * (m - 1) * (1 - p_mais)) + (p_L[2] * (L - 1) * (1 - p_mais)))
  theta_01 <- (c_desc_c * (p_mais * (1 - alfa)) / p_AS) + 
              (c_desc_nc * ((1 - p_mais) * (1 - beta)) / p_AS)
  phi_01 <- c_inspecao + xi_01 + theta_01
  
  # Cálculo para o estado (1,0)
  xi_10 <- c_nc * (
    p_AF[3] * sum(sapply(1:m, function(t) calcular.q(t, pi, m) * 
                             ((t - 1) * (1 - p_mais) + (m - t) * (1 - p_menos)))) +
    p_L[3] * sum(sapply(1:L, function(t) calcular.q(t, pi, L) * 
                             ((t - 1) * (1 - p_mais) + (L - t) * (1 - p_menos))))
  )
  theta_10 <- (c_desc_c * (p_menos * alfa) / (1 - p_AF)) + 
              (c_desc_nc * ((1 - p_menos) * (1 - beta)) / (1 - p_AF))
  phi_10 <- c_inspecao + xi_10 + theta_10 + c_ajuste
  
  # Cálculo para o estado (1,1)
  xi_11 <- c_nc * (
    p_AF[4] * sum(sapply(1:m, function(t) calcular.q(t, pi, m) * 
                             ((t - 1) * (1 - p_mais) + (m - t) * (1 - p_menos)))) +
    p_L[4] * sum(sapply(1:L, function(t) calcular.q(t, pi, L) * 
                             ((t - 1) * (1 - p_mais) + (L - t) * (1 - p_menos))))
  )
  theta_11 <- (c_desc_c * (p_menos * (1 - alfa)) / p_AF) + 
              (c_desc_nc * ((1 - p_menos) * beta) / p_AF)
  phi_11 <- c_inspecao + xi_11 + theta_11
  
  # Cálculo para o estado (2,0)
  xi_20 <- c_nc * (1 - p_menos) * (m - 1)
  theta_20 <- (c_desc_c * (p_menos * alfa) / (1 - p_AF)) + 
              (c_desc_nc * ((1 - p_menos) * (1 - beta)) / (1 - p_AF))
  phi_20 <- c_inspecao + xi_20 + theta_20 + c_ajuste
  
  # Cálculo para o estado (2,1)
  xi_21 <- c_nc * (1 - p_menos) * (m - 1)
  theta_21 <- (c_desc_c * (p_menos * (1 - alfa)) / p_AF) + 
              (c_desc_nc * ((1 - p_menos) * beta) / p_AF)
  phi_21 <- c_inspecao + xi_21 + theta_21
  
  # Retorna os custos de todos os estados
  return(list(
    phi_00 = phi_00, phi_01 = phi_01,
    phi_10 = phi_10, phi_11 = phi_11,
    phi_20 = phi_20, phi_21 = phi_21
  ))
}
```
...
val.p.AS.L <- calc.prob.sob.controle.L(pe, L.teste)
val.p.AF.L <- calc.prob.fora.controle.L(pe, L.teste)
val.p.AS.m <- calc.prob.sob.controle.m(pe, m.teste)
val.p.AF.m <- calc.prob.fora.controle.m(pe, m.teste)
...
matriz.P <- calc.matriz.prob.markov(val.p.AS, val.p.AF, val.p.AS.L, val.p.AF.L, val.p.AS.m, val.p.AF.m)
matriz.P

PI <- calcular_pi(matriz.P)
p.L <- calcular_p_L(matriz.P, PI)

PI
p.L

# Chamada da função com os parâmetros fornecidos
resultados <- calcular.custos(
  ...
  c_nc = c.nc, p_AS = val.p.AS, p_AF = val.p.AF, p_L = p.L,
