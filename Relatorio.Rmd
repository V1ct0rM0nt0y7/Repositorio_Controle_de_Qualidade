...
calcular.distribuicao.estacionaria.AV <- function(P) {
  ## Verificar se a matriz é quadrada
  if (!is.matrix(P) || nrow(P) != ncol(P)) {
    stop("A matriz de transição deve ser quadrada.")
  }
  ...
  if (!all(rowSums(P) == 1)) {
    stop("As linhas da matriz de transição devem somar 1 (matriz estocástica).")
  }
  
  ## Calcular os autovalores e autovetores
  eigen_decomp <- eigen(t(P))
  
  ## Localizar o autovalor igual a 1
  idx <- which(abs(eigen_decomp$values - 1) < .Machine$double.eps)
  if (length(idx) != 1) {
    stop("Não foi possível identificar um autovalor igual a 1.")
  }
  
  ## Selecionar o autovetor associado ao autovalor 1
  pi <- Re(eigen_decomp$vectors[, idx]) ## Parte real do autovetor
  
  ## Normalizar o autovetor para que a soma seja 1
  pi <- pi / sum(pi)
  
  ## Nomear os elementos de pi (opcional)
  names(pi) <- paste0("pi.", 1:length(pi))
...
##### **Funções dos Custo Médio**

```{r funcoes10, echo = T, include = T, warning = F}
# Função para calcular o custo médio C(m, L)
calcular_custo_medio <- function(L, m, pi, resultados, p_L, c_insp) {
  
  # Verificando os resultados
  print("Verificando os resultados (xi, theta, phi) para cada estado:")
  print(resultados)
  
  # Numerador: Somatório das distribuições estacionárias ponderadas pelos custos
  numerador <- sum(pi * (c_insp + resultados$xi + resultados$theta + resultados$phi))
  
  # Exibir o numerador para verificar se está correto
  print(paste("Numerador:", numerador))
  
  # Denominador: Somatório ponderado de pi pelos termos de amostragem L e m
  denominador <- sum(pi * ((L - 1) * p_L + (m - 1) * (1 - p_L)))
  
  # Exibir o denominador para verificar se está correto
  print(paste("Denominador:", denominador))
  
  # Cálculo final do custo médio
  custo_medio <- numerador / denominador
  
  # Verificando o custo médio
  print(paste("Custo Médio:", custo_medio))
...
```{r funcoes11, echo = F, include = F, warning = F}
...
PI.AV <- calcular.distribuicao.estacionaria.AV(matriz.P) ## Calcula a distribuição estacionária usando o método LU

summary(microbenchmark(AV = calcular.distribuicao.estacionaria.AV(matriz.P), 
                       LU = calcular.distribuicao.estacionaria.LU(matriz.P), 
                       QR = calcular.distribuicao.estacionaria.QR(matriz.P), times = 200)) ## Compara o tempo de execução entre as funções com microbenchmark

p.L.AV <- calcular.pL(matriz.P, PI.AV) ## Calcula a probabilidade de transição usando QR
...
p.L.AV; p.L.LU; p.L.QR; ## Compara os valores

cat("Matriz de Transição (P):\n"); matriz.P ## Imprime a matriz de transição P
cat("\nDistribuição Estacionária:\n", PI.QR) ## Imprime a distribuição estacionária
...
custo_medio <- calcular_custo_medio(
  L = L.teste, 
  m = m.teste, 
  pi = pe, 
...
valores_L <- seq(100, 600, by = 5) # Ajuste os intervalos conforme necessário
valores_m <- seq(5, 100, by = 5)
...
for (L_teste in valores_L) {
  for (m_teste in valores_m) {
    ...
    resultados_gerais <- rbind(resultados_gerais, data.frame(L = L_teste, m = m_teste, custo_medio = custo_medio))
  }
}

# Identificar a combinação que minimiza o custo
melhor_comb <- resultados_gerais[which.min(resultados_gerais$custo_medio), ]
